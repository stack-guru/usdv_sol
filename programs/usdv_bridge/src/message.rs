...
pub const WORMHOLE_MESSAGE_MAX_LENGTH: usize = 512;
...
impl AnchorSerialize for WormholeMessage {
    fn serialize<W: io::Write>(&self, writer: &mut W) -> io::Result<()> {
        match self {
            WormholeMessage::Alive { program_id } => {
                PAYLOAD_ID_ALIVE.serialize(writer)?;
                program_id.serialize(writer)
            }
            WormholeMessage::Hello { message } => {
                if message.len() > WORMHOLE_MESSAGE_MAX_LENGTH {
                    Err(io::Error::new(
                        io::ErrorKind::InvalidInput,
                        format!("message exceeds {WORMHOLE_MESSAGE_MAX_LENGTH} bytes"),
                    ))
                } else {
        ...
        match u8::read(reader)? {
            ...
            PAYLOAD_ID_HELLO => {
                ...
                if length > WORMHOLE_MESSAGE_MAX_LENGTH {
                    ...
                    Ok(WormholeMessage::Hello { message: buf })
                }
            }
            _ => Err(io::Error::new(
                io::ErrorKind::InvalidInput,
...
mod test {
    ...
    fn test_message_alive() -> Result<()> {
        ...
        match WormholeMessage::deserialize(&mut encoded.as_slice())? {
            WormholeMessage::Alive { program_id } => {
